...
def extract_hyperlinks(line, context_size=10):
	...
	for link in hyp_matches:
		...
		start_index = 0
		if hyperlink['mention'] in mentions.keys():     # avoid repeating context for similar mentions in one line
		...
		valid_left_words = list(filter(lambda w: w in word2id.keys(), left_words))
		...
		valid_right_words = list(filter(lambda w: w in word2id.keys(), right_words))
		...
		with open(wiki_path, 'r',encoding='utf8') as inf:
			...
			for line in inf:
				for hyp in extract_hyperlinks(line, context_size=context_size):
					...
					if hyp['name'] in name2wikiid.keys():   # link to valid entity
